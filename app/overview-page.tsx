...
type Protocol = { name: string; module?: string; category?: string; logo?: string; total24h?: number; total7d?: number; total30d?: number; change_1d?: number };
...
type Launchpad = { slug: string; name: string; available: boolean; fees24h: number | null; fees7d: number | null; change24h: number | null };
...
export default function OverviewPage({ language, isDark, onNavigate }: Props) {
  ...
  return <div className="content overview-content">
    ...
    <div className="overview-metrics">
      ...
      <article className="overview-metric-card"><div className="overview-metric-label"><span>{t('TRACKED PROTOCOLS', '已追踪协议')}</span><Sparkles size={15}/></div><strong>{data?.metrics.protocolCount ?? '—'}</strong><div className="overview-metric-foot count-foot"><span>{t('Protocols on Arc', 'Arc 链协议')}</span><a href="https://defillama.com/chain/arc" target="_blank" rel="noreferrer">{t('View list', '查看列表')} <ExternalLink size={11}/></a></div></article>
    </div>
    ...
    <section className="overview-chart-card">
      ...
      <div className="overview-chart-summary"><span>{currentMetric.label}<b>{currency(currentMetric.value)}</b></span><span>{t('24h change', '24 小时变化')}<b className={(currentMetric.change ?? 0) < 0 ? 'negative' : 'positive'}>{percent(currentMetric.change)}</b></span><span className="overview-chart-source">{t('Daily data · DefiLlama', '每日数据 · DefiLlama')}</span></div>
    </section>

    <div className="overview-lower-grid">
      <section className="overview-panel"><div className="overview-panel-head"><div><div className="section-kicker">{t('ON-CHAIN FEES', '链上费用')}</div><h3>{t('Top protocols · 24h', '协议费用榜 · 24 小时')}</h3></div><a href="https://defillama.com/chain/arc" target="_blank" rel="noreferrer">{t('All protocols', '全部协议')} <ExternalLink size={12}/></a></div>
        <div className="overview-table-scroll"><table className="overview-table"><thead><tr><th>#</th><th>{t('PROTOCOL', '协议')}</th><th>{t('CATEGORY', '类别')}</th><th>{t('FEES · 24H', '费用 · 24 小时')}</th><th>{t('FEES · 7D', '费用 · 7 天')}</th></tr></thead><tbody>{data?.topProtocols.map((protocol, index) => <tr key={`${protocol.name}-${index}`}><td className="overview-rank">{String(index + 1).padStart(2, '0')}</td><td><div className="overview-protocol-name"><i>{protocol.name.slice(0, 1)}</i><b>{protocol.name}</b></div></td><td className="overview-category">{protocol.category ?? '—'}</td><td className="overview-money">{currency(protocol.total24h)}</td><td className="overview-money">{currency(protocol.total7d)}</td></tr>)}{!data && <tr><td colSpan={5} className="overview-loading">{t('Loading protocol fees…', '正在加载协议费用…')}</td></tr>}{data?.topProtocols.length === 0 && <tr><td colSpan={5} className="overview-loading">{t('No protocol fee data available.', '暂无协议费用数据。')}</td></tr>}</tbody></table></div>
      </section>
