...
import { ChevronDown, Compass, Eye, LayoutDashboard, Menu, Moon, Sparkles, Sun, X } from 'lucide-react';
...
export default function Home(){
 ...
 return <main className="shell">
  <aside className={`sidebar ${mobileOpen?'mobile-open':''}`}>
   ...
   <nav>{[['Overview',LayoutDashboard],['Ecosystem',Compass],['Launchpad',Sparkles],['Watchlist',Eye]].map(([label,Icon]:any)=><button key={label} className={`nav-item ${active===label?'selected':''}`} onClick={()=>{setActive(label);setMobileOpen(false)}}><Icon size={17}/><span>{tr(label,({Overview:'概览',Ecosystem:'生态',Launchpad:'发射台',Watchlist:'关注列表'} as Record<string,string>)[label]||label)}</span>{label==='Ecosystem'&&<span className="nav-count">01</span>}</button>)}</nav>
