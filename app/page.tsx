...
export default function Home(){
 ...
 const [ecosystemProjectCount,setEcosystemProjectCount]=useState<number|null>(null);
 ...
 useEffect(()=>{fetch('/api/ecosystem-projects',{cache:'no-store'}).then(r=>r.ok?r.json():[]).then((projects:unknown)=>{if(Array.isArray(projects))setEcosystemProjectCount(projects.length)}).catch(()=>setEcosystemProjectCount(null))},[]);
 ...
 return <main className="shell">
  <aside className={`sidebar ${mobileOpen?'mobile-open':''}`}>
   ...
   <nav>{[['Overview',LayoutDashboard],['Ecosystem',Compass],['Launchpad',Sparkles]].map(([label,Icon]:any)=><button key={label} className={`nav-item ${active===label?'selected':''}`} onClick={()=>{setActive(label);setMobileOpen(false)}}><Icon size={17}/><span>{tr(label,({Overview:'概览',Ecosystem:'生态',Launchpad:'发射台'} as Record<string,string>)[label]||label)}</span>{label==='Ecosystem'&&ecosystemProjectCount!==null&&<span className="nav-count">{String(ecosystemProjectCount).padStart(2,'0')}</span>}</button>)}</nav>
